...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"ticketNo\":\"CS-2026-1001\",\"priority\":\"P2\",\"elapsedMinutes\":90,\"slaMinutes\":240,\"firstResponseMinutes\":12,\"resolved\":false,\"escalated\":false}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"ticketNo\":\"CS-2026-1001\",\"priority\":\"P1\",\"elapsedMinutes\":400,\"slaMinutes\":120,\"firstResponseMinutes\":100,\"resolved\":false,\"escalated\":false}"))
